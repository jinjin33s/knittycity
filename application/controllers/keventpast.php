...
class Keventpast extends MY_Controler {
	...
	function overview($page = 1)
	{
		...
		$this->view->add_data('kevent_list', $this->kevent_model->get_kevent_list_by_past($page));

                //show_knitty($page_title = '', $view_name = '', $template_name, $debug = FALSE, $simple = FALSE, $add_data = TRUE)
                 $this->view->show_knitty('kevent', 'kevent_overview_past','event_template');
	}
	...
	function view($event_id = FALSE)
	{
		if($event_id == FALSE)
		{
			redirect('admin/keventpast/overview');
		}
		else
		{
