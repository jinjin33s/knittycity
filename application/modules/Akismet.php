...
class Akismet
{
	...
	var $module_name = 'Akismet';
	var $module_description = '';

	function __construct($config = FALSE)
	{
		$this->CI =&get_instance();
		$this->user_agent = "DBlog/".$this->CI->config->item('dblog_version')." | Akismet/".$this->module_version;
		if(is_array($config))
		{
			$this->init($config);
		}
	...
	function get_info()
	{
		return array('module_name' => $this->module_name, 'module_description' => $this->module_description, 'module_version' => $this->module_version, 'module_url' => '');
	}

	function get_actions()
	{
		return array('comment_submit_spam_check' => 'comment_is_spam', 'admin_comment_spam' => 'submit_spam', 'admin_comment_unspam' => 'submit_ham');
	}
}